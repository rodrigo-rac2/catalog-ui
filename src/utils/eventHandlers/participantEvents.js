...
export function setupParticipantEventHandlers(apiBaseUrl) {
  ...
  document.getElementById("loadParticipants").addEventListener("click", async () => {
    ...
    if (participantsList.style.display === "block") {
      ...
      try {
        ...
        displayStatusMessage("participants", `Failed to load participants: ${error.message}`, "error");
      }
    }
  });
  ...
  document.addEventListener("editParticipant", async (event) => {
    ...
    try {
      await participantService.updateParticipant(participantId, { name: newName });
      displayStatusMessage("participants", "Participant updated successfully", "success");
      const participants = await participantService.fetchParticipants();
      displayParticipants(participants, apiBaseUrl);
    } catch (error) {
      console.error("Error updating participant:", error);
      displayStatusMessage("participants", `Error updating participant: ${error.message}`, "error");
    }
  });
  ...
  document.getElementById("participants-list").addEventListener("click", async (event) => {
    ...
    if (target.classList.contains("delete-btn")) {
      ...
      try {
        ...
        displayStatusMessage("participants", "Participant deleted successfully", "success");
      } catch (error) {
        console.error("Error deleting participant:", error);
        displayStatusMessage("participants", `Error deleting participant: ${error.message}`, "error");
      }
    }
  });
  ...
  document.getElementById("participant-form").addEventListener("submit", async (event) => {
    ...
    try {
      ...
      if (result) {
        displayStatusMessage("participants", `Participant ${participantId ? "updated" : "added"} successfully!`, "success");
        const participants = await participantService.fetchParticipants();
        displayParticipants(participants, apiBaseUrl);
        form.reset();
      ...
      displayStatusMessage("participants", `Failed to ${participantId ? "update" : "add"} participant: ${error.message}`, "error");
    }
  });
}
