...
export function displayParticipants(participants) {
  ...
  if (!participantsList) {
    console.error('displayBooks: No element found with ID "participants-list"');
    return;
  }
  ...
  participants.forEach((participant) => {
    ...
    openBtn.onclick = () =>
      toggleParticipantDetails(participant, participantItem);
...
function toggleParticipantDetails(participant, participantItem) {
  ...
  const existingForm = participantItem.querySelector(
    `form.${participantFormClassName}`
  );
  if (existingForm) {
    existingForm.remove();
  } else {
...
function toggleEditForm(participant, participantItem) {
  ...
  if (existingForm) {
    ...
    form.onsubmit = (e) =>
      handleEditSubmit(e, participant.participantid, participantItem);
...
function handleEditSubmit(event, participantId, participantItem) {
  ...
  const customEvent = new CustomEvent("editParticipant", {
    detail: { participantId, newName },
  });
